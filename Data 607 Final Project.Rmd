...
The National Highway Traffic Safety Administration makes their recommendation using a star rating system. More Stars mean Safer Cars. 5-Star Safety Ratings measure the crashworthiness and rollover safety of vehicles. Five stars is the highest rating, one is the lowest. However, some of the vehicles getting a 5-star rating could be cost prohibitive for some consumers or to provide a larger variety of choices. In this instance, a more detailed analysis of the injury criteria and attributes are warranted.  
...
The National Highway Traffic Safety Administration Recommendations provide a basis for vehicle safety for crashworthiness and rollover. The recommendation image provides a baseline for head, leg and chest injury.  In summary, the close the value is to absolute 0, the more likely the occupant will have less severe injury.   
...
### In order to simplify our analysis we have decided to separete head injury criterion, left/right femur load and compare them against the attributes we thought were most relevant. We will add a column that averages the HIC, LFL and RFL by make, year, body type of the car and occupant location in the car.
...
### Data by Car Make Sample

```{r}
head(attrMake)
```

### The car make that has the highest number of crash tests in our data is Ford, and is then followed by Toyota, Honda, Chevrolet, Dodge, and Nissan to name a few.
...
cn <- dbConnect(drv      = RMySQL::MySQL(), 
                ...
                port     = 3306, 
                dbname   = "data607finalproject")
data3 <- dbGetQuery(cn, "SELECT 
COUNT(RD.MAKED) AS TotalMakes,
RD.MAKED
FROM
(
SELECT
...
```{r}
ggplot(attrMake, aes(x=reorder(MAKED, -Count), y=Count)) + geom_bar(stat="identity", width = 0.5, fill = "tomato2") + labs(x = "Car Make", y = "Count", title = "Number of Crash Tests by Car Make") + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=8))
```

### In the graphs below we can see from the crash tests that among the safest car makes from a head and leg injury perspective we find Porsche, Tesla, Mini and Volkswagen among others.
...
### Injury by Car Body Type

```{r}
attrBody <- data2 %>% group_by(BODYD) %>% summarise("Average HIC" = mean(HIC), Count = n())
attrBody_lfl <- data2 %>% group_by(BODYD) %>% summarise("Average LFL" = mean(LFL), Count = n())
attrBody_rfl <- data2 %>% group_by(BODYD) %>% summarise("Average RFL" = mean(RFL), Count = n())
```

### Data by Car Body Type Sample

```{r}
head(attrBody)
```

### According to the graphs below, among the safest car body types from a head and leg injury perspective we find that three door coupes, five door hatchbacks, and 4 door pickups have the lowest injury averages.

```{r}
ggplot(attrBody, aes(x=reorder(BODYD, `Average HIC`), y=`Average HIC`)) + geom_bar(stat="identity", width = 0.5, fill = magma(16)) + labs(x = "Body Type", y = "AVG HIC", title = "Average HIC by Car Body Type") + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=8))

ggplot(attrBody_lfl, aes(x=reorder(BODYD, -`Average LFL`), y=`Average LFL`)) + geom_bar(stat="identity", width = 0.5, fill = magma(16)) + labs(x = "Body Type", y = "AVG LFL", title = "Average LFL by Car Body Type") + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=8))

ggplot(attrBody_rfl, aes(x=reorder(BODYD, -`Average RFL`), y=`Average RFL`)) + geom_bar(stat="identity", width = 0.5, fill = magma(16)) + labs(x = "Body Type", y = "AVG RFL", title = "Average RFL by Car Body Type") + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=8))
```
...
### Data by Occupant Location Sample

```{r}
head(attrOcc)
```

### According to the graphs below, one of the safest places to sit in a car in the unfortunate event of a crash is the left front seat. In other words, you are the most safe when you are the driver. The second safest place to sit is in the front passenger seat.
...
### Below we have another perspective for head injury criterion by occupant location looking at the data through boxplots. We can see that our boxplots agree with our analysis above, the two safest places to sit in a car are in the driver seat and front passenger seat.
...
### We can also look at our data by comparing car make, body type and head injury criterion in the same graph in order to have a better visualization of our aggregate data. 

**Please Note: We have removed an observation (Chevrolet convertible) that was very far from the rest of the data in order to make the graph more readable.**

```{r}
attrMakeBody <- data2 %>% group_by(MAKED, BODYD) %>% filter (MAKED != "CHEVROLET" | BODYD != "CONVERTIBLE") %>% summarise("Average HIC" = mean(HIC), Count = n())

ggplot(attrMakeBody, aes(MAKED, `Average HIC`)) + geom_point(aes(color = BODYD)) + labs(x = "Car Make", y = "AVG HIC", title = "Average HIC by Car Make and Body Type") + theme(axis.text.x = element_text(angle = 60, hjust = 1, size=6), legend.title = element_text(size = 8), legend.text = element_text(size = 6)) + scale_color_discrete(name = "Body Type")
